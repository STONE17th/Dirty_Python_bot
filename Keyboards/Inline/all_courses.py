...
def create_ikb_class_navigation(menu: str, size: int, table: str, current_id: int, admin: bool,
    ...
    btn_prev = IKB(text='<<<', callback_data=crt_callback(menu, table, prev_id))
    btn_next = IKB(text='>>>', callback_data=crt_callback(menu, table, next_id))
    btn_back = IKB(text='Назад', callback_data=main_menu.new(menu='', button='all_courses'))
    btn_lecture_edit = IKB(text='Изменить', callback_data=crt_callback('edit_class', table, current_id))
    btn_finalize_course = IKB(text='Завершить', callback_data=crt_callback('finalize_course', table, current_id))
    btn_purchase = IKB(text='Купить', callback_data=crt_callback('purchase', table, current_id))
    ...
    if admin:
        ...
        if not ((user_lectures and f'{table}:{current_id}' in user_lectures) or (
                user_courses and table not in user_courses)):
...
def create_ikb_individual():
    ikb_individual = InlineKeyboardMarkup(row_width=2)
    btn_want = IKB(text='Оставить заявку', callback_data=crt_callback('want'))
    btn_back = IKB(text='Назад', callback_data=main_menu.new(menu='', button='all_courses'))
